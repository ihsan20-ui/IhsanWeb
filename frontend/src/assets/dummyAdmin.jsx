...
import { FiClock, FiTruck, FiCheckCircle, FiShoppingCart } from "react-icons/fi";
import { GiForkKnifeSpoon, GiChefToque } from "react-icons/gi";

// Example Input component – replace with your own
const Input = ({ label, name, type = "text", value, onChange }) => (
  <div className="flex flex-col space-y-2">
    <label className="text-amber-200 font-semibold">{label}</label>
    <input
      type={type}
      name={name}
      value={value}
      onChange={onChange}
      className="p-2 rounded-lg bg-[#2D1B0E] border border-amber-700 text-amber-100"
    />
  </div>
);

// ================== CHECKOUT PAGE ==================
const CheckoutPage = ({ subtotal, tax, cartItems }) => {
...
// ================== NAVBAR ==================
const Navbar = ({ navLinks, totalItems, renderDesktopAuthButton, renderMobileAuthButton, showLoginModal, handleLoginSuccess }) => {
...
const ContactUs = () => {
  ...
  return (
    <div className="p-6 bg-[#2D1B0E] rounded-2xl text-amber-100">
      ...
      <Input label="Dish" name="dish" value={formData.dish} onChange={(e) => setFormData({ ...formData, dish: e.target.value })} />
      <Input label="Query" name="query" value={formData.query} onChange={(e) => setFormData({ ...formData, query: e.target.value })} />
      <button
...
export { CheckoutPage, Navbar, ContactUs };
