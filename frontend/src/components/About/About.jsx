...
import { features,stats ,teamMembers ,aboutfeature } from '../../assets/dummydata'
     ...
     overflow-hidden relative'>
        ...
        <section className=' py-12 px-4 md:px-8 relative'>
            <div className=' max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 md:gap-12'>
               {features.map((f,i) =>{
                const Icon =f.icon;
                return(
                    <motion.div key={f.id} initial={{opacity:0,y:30}} whileInView={{opacity:1,y:0}} viewport={{once:true,margin:"0px 0px -100px 0px"}}
                    transition={{delay:i* 0.2}} className=' relative group'>
                        <div className=' absolute -indent-1 bg-gradient-to-br from-amber-600/30 to-amber-500/30 rotate-3
                         ...
                         <div className=' relative bg-[#3c2a21]/90 backdrop-blur-lg rounded-3xl overflow-hidden
                          border border-amber-600/30 hover:border-amber-500 transition-all duration-300 h-full'>
                            <div className=' relative h-64 overflow-hidden'>
                                <motion.img src={f.img} alt={f.title} className=' w-full h-full object-cover' initial={{ scale:1}} whileHover={{scale:1.05}}
        ...
        <div className=' max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8'>
            {stats.map((s,i) =>{
                const Icon =s.icon
                return(
                    <motion.div key={s.label} initial={{opacity:0, y:50}} whileInView={{opacity:1,y:0}} transition={{delay:i*0.2, type:'spring'}} className=' relative group h-48 ' onHoverStart={()=> setHoveredStat(i)}
                            ...
                            <div className=' relative h-full bg-[#3c2a21]/40 backdrop-blur-lg rounded-xl
                             border-2 border-amber-600/30 p-6 overflow-hidden transition-all duration-300'>
                                ...
                                <div className=' relative z-10 h-full flex flex-col items-center justify-center'>
                                    ...
                                    <div className=' text-4xl font-bold mb-1 bg-clip-text bg-gradient-to-r from-amber-200
                                     to-amber-400 text-transparent '>
                                        {s.number}
                                      </div>
                               ...
                               <motion.div className=' absolute inset-0 bg-amber-900/10  rounded-xl' initial={{opacity:0}} 
        ...
        <section className=' py-16 px-4 md:px-8 relative'>
            <div className=' max-w-7xl  mx-auto'>
                <motion.h2 initial={{opacity:0,y:20}} whileInView={{opacity:1,y:0}} className=' text-4xl font-serif sm:text-5xl md:text-6xl font-bold text-center
                 mb-12 text-amber-100' > Meet Our <span className=' text-amber-500'>Culinary Artists</span>
                 </motion.h2>
                 <div className=' grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 lg:gap-12'>
                    {teamMembers.map((m,i) =>(
                        <motion.div key={i}  initial={{opacity:0,y:30}} whileInView={{opacity:1,y:0}} viewport={{once:true,margin:"0px 0px -100px 0px"}}
