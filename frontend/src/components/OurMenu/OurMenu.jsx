...
const OurMenu = () => {
  ...
  const fetchMenu = async () => {
    try {
      const res = await axios.get('https://ihsanweb-backend.onrender.com/api/items'); // <-- add await
      ...
      <div className=' flex items-center  gap-2'>
         {quantity > 0 ?(
            ...
            <button className=' w-8  h-8 rounded-full bg-amber-900/40 flex items-center
             justify-center hover:bg-amber-800/50 transition-colors'   onClick={() =>
                              qty > 1
             ...
             <span className=' w-8 text-center text-amber-100'>
               {quantity}
                 ...
                 <button className=' w-8  h-8 rounded-full bg-amber-900/40 flex items-center
             justify-center hover:bg-amber-800/50 transition-colors' onClick={() => updateQuantity(cartEntry._id, quantity + 1)}>
            ...
            className=' bg-amber-900/40 px-4 py-1.5 rounded-full font-cinzel  text-xs uppercase
             sm:text-sm tracking-wider transition-transform duration-300
